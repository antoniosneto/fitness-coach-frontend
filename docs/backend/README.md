...
- **Implementado:** Auth (signup, login, recuperação de senha REQ-AUTH-003), Onboarding (PUT profile, PUT goals com validação 1,5%/semana, **PUT training-preferences** com `machines_only` DD-ENT-TRAININGPREFERENCE), **Plans** (POST /plans/weekly com Motor de Metas, estrutura de treino semanal), JWT Guard/Strategy, Prisma 7 com adapter pg, banco local (Docker ou Homebrew), seed TACO, collection Postman.
...
- **Pendente (próximos passos):** Preferências de esportes (DD-ENT-SPORTPREFERENCE) se definido no contrato, testes e2e dos SCN-*.
